...
expand_decapsulation_key(const XWingSecretKey &sk) {
  ...
  PQCLEAN_MLKEM768_CLEAN_crypto_kem_keypair_derand(m_pk.b.data(), m_sk.b.data(),
                                                   expanded.data());
 ...
 */
XWingSharedSecret combiner(const MSharedSecret &m_ss, const XSharedSecret &x_ss,
                           const XCipherText &x_ct, const XPublicKey &x_pk) {
...
encapsulate(const XWingPublicKey &xwing_pk) {
  MPublicKey m_pk;
  XPublicKey x_pk;
  XCipherText x_ct;
  XSharedSecret x_ss;
 ...
 */
XWingSharedSecret decapsulate(const XWingCipherText &xwing_ct,
                              const XWingSecretKey &xwing_sk) {
  MSecretKey m_sk;
  XSecretKey x_sk;
  MPublicKey m_pk;
  XPublicKey x_pk;
  ...
  std::copy(xwing_ct.b.begin(), xwing_ct.b.begin() + M_CT_BYTES,
            m_ct.b.begin());
  std::copy(xwing_ct.b.begin() + M_CT_BYTES,
            xwing_ct.b.begin() + M_CT_BYTES + X_CT_BYTES, x_ct.b.begin());

  PQCLEAN_MLKEM768_CLEAN_crypto_kem_dec(m_ss.b.data(), m_ct.b.data(),
                                        m_sk.b.data());
 ...
 */
std::tuple<XWingSecretKey, XWingPublicKey>
generate_key_pair_derand(const XWingSecretKey &xwing_sk) {
  XWingPublicKey xwing_pk;
  XSecretKey x_sk;
  XPublicKey x_pk;
  MSecretKey m_sk;
...
/**
 * The following schematic is recreated from the FIPS-203 definition for a
 * simplified key establishment process. The left boxes (KeyGen, Decaps) are
 * implied to be Alice's and the right box (Encaps) is implied to be Bob's. The
 * encapsulation key is the public key for this scheme, whereas the
 * decapsulation key is the private key.
 *
 * In effect, Alice will generate both, pass the encapsulation key to parties
 * looking to establish shared keying material, and then receive some the
 * ciphertext of this shared keying material. Upon receiving, it will
 * decapsulate it (decrypt), then using this shared keying material in a
 * symmetric scheme.
 *
 * In this specific file, we implement X-Wing (as of its 8th revision), which is
 * a KEM that uses a so-called 'belt and suspenders' approach to combine the use
 * of X25519 (a well studied but non-quantum-resistant elliptic curve algorithm)
 * and ML-KEM-768 (a quantum resistant KEM).
 *
 * ┌──────────────────────┐
 * │                      │
 * │        KeyGen        │
